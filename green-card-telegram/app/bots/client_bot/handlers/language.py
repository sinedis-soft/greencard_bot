...
@router.message(F.text.in_({"/language", "/lang", "🌐 Язык", "🌐 Language"}))
async def language_menu(message: Message, i18n: I18nService, lang_store: dict[int, str], default_language: str) -> None:
    lang = lang_store.get(message.from_user.id, default_language)
    await message.answer(i18n.get_text(lang, "language.select"), reply_markup=language_keyboard())
